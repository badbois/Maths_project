...
class Object {
    ...
    public:
    // Default constructor
        Object(): shape(Shape::CIRCLE), color(p6::Color(1.f, 0.f, 0.f)), rotating_direction(Rotating_direction()), position(Position2D(0,0)){};

    //Constructor
        explicit Object(const std::vector<p6::Color> &colors, const int nb_objects_by_line):
    ...
    //Setters
        void set_position(const Position2D &pos) {
            position = pos;
        }
    ...
    //Getters
        ...
        p6::Color get_color() const {
            return color;
        }

        int get_rotating_direction() const {
            return rotating_direction.direction;
        }
...
inline void set_all_positions(std::vector<Object> &objects, std::vector<Position2D> &positions){
    for(int i = 0; i < objects.size(); i++){
        objects[i].set_position(positions[i]);
    }
}

inline std::vector<Position2D> create_positions(std::vector<Object> &objects, int nb_objects_by_line){
    std::vector<Position2D> positions;
    while(positions.size() < objects.size()){
        const Position2D tmp=Position2D(random_position(nb_objects_by_line-1), random_position(nb_objects_by_line-1));
